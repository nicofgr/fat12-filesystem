...
u32 allocateFAT(){ // Um cluster será escrito em um metacluster
        puts("Alocando espaço na file table");
        ...
        int lastIndex = getFirstEmptyClusterIndex();
        printf("Last Index: %d Cluster\n", lastIndex);
        printf("Last Index: %d Mega Cluster\n", lastIndex/2);
        metaClus = getNormalizedMetaCluster(lastIndex/2);
        printf("%.3x ", metaClus.nextCluster1);
        printf("%.3x\n", metaClus.nextCluster2);
        u32 lastEntryAddress = FT1_START+(lastIndex*1.5);
        printf("Address: (0x%x)\n", lastEntryAddress);
        for(int i = 0; i < 3; i++)
...
void printHumanReadableFileTable(){
        ...
        for(int i = 0; i < lastClusterIndex; i++){
                u32 cluster = getNormalizedClusterByIndex(i);
                printf("%.3x(%x) ", cluster, (33+i-2)*512);
        }
        printf("\n");
}
